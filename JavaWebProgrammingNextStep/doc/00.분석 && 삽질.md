# 흙흙흙흙
	...
	- COLUMN 
		...
		- CREATED_DATE 		// 날짜
		ㅁㄴㅇㅁㄴㅇ
	- 게시글에 첫 번째 댓글은 PARENT_REPLY_ID에 NULL이 들어가고 REPLY_LEVEL이 1로 되어 있다. 
	따라서 ORDER BY절을 통해 PARENT_REPLY_ID가 NULL이면 REPLY_ID를 넣어주고, NULL이 아니라면 PARENT_REPLY_ID를 그대로 넣어준다.
	ex)
	SELECT
	  A.*
	FROM
	...
	% ORDER BY로 정렬할 때 정렬 순서에 따라 영향을 미친다.
	  
	- 근데 문제는 첫 번째 댓글 등록 날짜(CREATED_DATE)를 기준으로 먼저 등록한 댓글(첫 댓글) 기준으로 오름차순이 되어야 하는데, 이 순서가 제대로 정렬되지 않는다.
	일반적인 경우에는 댓글 등록 순서에 따라 REPLY_ID가 순서대로 등록되지만, 저 상태로
	정렬이 되는지 시도해보자. (...나중에..)
